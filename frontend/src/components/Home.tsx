    ...
    <div style={{ 
      ...
      <section ref={s1.ref} style={{ padding: sectionPadding, background: G.s100 }}>
        <div style={{ 
          ...
          {/* left */}
          <div className={s1.vis ? 'fade-up' : ''} style={{ opacity: s1.vis ? 1 : 0, transition: 'opacity 0.6s ease' }}>
          ...
          {!isMobile && (
            <div className={s1.vis ? 'slide-left' : ''} style={{ opacity: s1.vis ? 1 : 0, position: 'relative', transition: 'opacity 0.6s ease' }}>
      ...
      <section ref={s8.ref} style={{ 
        ...
        <div style={{ maxWidth: containerMaxWidth, margin: '0 auto' }}>
          ...
          <div style={{ 
            ...
            {[
              { name: 'Sarah Chen', role: 'Homeowner · Nairobi', flag: '🇰🇪', q: 'Found the perfect sitter for my 3-month trip. Daily updates gave me complete peace of mind.', delay: '0s' },
              { name: 'James Wilson', role: 'Digital Nomad · Sydney', flag: '🇦🇺', q: 'House sitting let me live like a local in 8 countries this year. The verified community makes every arrangement feel completely safe.', delay: '.1s' },
              { name: 'Maria Rodriguez', role: 'Student · London', flag: '🇬🇧', q: 'Fully furnished, fast wifi, housekeeping included. The flexible contract meant I could focus entirely on my studies.', delay: '.2s' },
        ...
        {(stats ? [
          ...
          >
            {s.v ? (
              ...
              <div className="shimmer" style={{ height: isMobile ? 40 : 60, borderRadius: 8, background: 'linear-gradient(90deg, #f0f0f0 0%, #e4e4e4 40%, #f0f0f0 80%)', backgroundSize: '600px 100%', animation: 'shimmer 1.6s infinite linear' }}/>
            )}
          </div>
        ))}
      </div>
...
const SkeletonCard: React.FC<{ h: number }> = ({ h }) => (
  <div className="shimmer" style={{ height: h, borderRadius: 18, background: 'linear-gradient(90deg, #f0f0f0 0%, #e4e4e4 40%, #f0f0f0 80%)', backgroundSize: '600px 100%', animation: 'shimmer 1.6s infinite linear' }}/>
);
